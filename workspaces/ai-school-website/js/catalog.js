...
(function () {
  ...
  /* flatten every lesson into a searchable row */
  const rows = [];
  PHASES.forEach((p) => p.lessons.forEach((l, i) => rows.push({
    phase: p.id, phaseName: p.name, idx: i,
    name: l.name, type: l.type, lang: l.lang || '—',
    live: l.status === 'complete', url: l.url || ''
  })));
  const userStatus = (r) => !r.live ? 'planned' : store.isDone(r.phase, r.idx) ? 'cleared' : 'open';
  ...
  function render() {
    ...
    body.replaceChildren(...out.map((r) => {
      ...
      return el('tr', null, [
        ...
        el('td', null, el(r.url ? 'a' : 'span', r.url ? { class: 'cat-name', href: r.url, target: '_blank', rel: 'noopener' } : { class: 'cat-name' }, r.name)),
